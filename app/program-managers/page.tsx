...
import { UserPlus, Edit, Trash2, Shield, Search, X, CheckCircle2, AlertCircle, ChevronDown, Phone } from "lucide-react";
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg font-semibold transition-colors shadow-sm cursor-pointer"
              >
                <UserPlus size={18} />
                Add Program Manager
              </button>
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 w-full xl:w-auto min-w-0">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 xl:flex-initial">
                      {/* Name */}
                      <div className="min-w-0 flex-1 xl:w-48 2xl:w-56 flex items-center gap-2">
                        <h3 className="text-base sm:text-lg font-semibold text-gray-800 truncate">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/program-managers/${pm._id}?from=${encodeURIComponent("/program-managers")}`);
                            }}
                      ...
                      {pm.programName && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 flex-shrink-0">
                          {pm.programName}
                        </span>
                      )}
                    </div>
                ...
                {expandedManager.includes(pm._id) && (
                  <div className="border-t border-yellow-300 bg-yellow-50 px-4 sm:px-6 py-3 sm:py-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-3 sm:mt-4">
                      <button
                        onClick={() => router.push(`/program-managers/${pm._id}?from=${encodeURIComponent("/program-managers")}`)}
                        className="flex-1 px-4 sm:px-6 py-2 bg-[#A65353] text-white cursor-pointer rounded transition-colors text-sm sm:text-base"
                      >
                        Overview
                      </button>
                      <button
                        onClick={() => router.push(`/program-managers/${pm._id}?edit=true&from=${encodeURIComponent("/program-managers")}`)}
                        className="flex-1 px-4 sm:px-6 py-2 bg-[#A65353] text-white cursor-pointer rounded transition-colors text-sm sm:text-base"
                      >
                        Edit
                      </button>
