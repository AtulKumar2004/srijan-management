...
export async function GET(req: NextRequest) {
  try {
    ...
    const requests = await MentorshipChangeRequest.find(query)
      .populate("participant", "-password")
      .populate("requestedHandledBy", "name email phone level")
    ...
    console.error("GET_MENTORSHIP_CHANGE_REQUESTS_ERROR:", error);
    return NextResponse.json(
